...
export interface StudentGetAllRequest extends MyPagedRequest {
  ...
  firstLast?: string; //Upit za pretragu ime prezime/prezime ime (ili samo ime ili samo prezime ako nema space-a)
}
...
export class StudentGetAllEndpointService
  ...
  handleAsync(request: StudentGetAllRequest, useCache: boolean = false, cacheTTL: number = 300000) {
    let key = `students-${request.pageNumber}-${request.pageSize}-${request.firstLast ?? ''}-${request.q ?? ''}`;
    if(useCache) {
      let res = this.cacheService.get<MyPagedList<StudentGetAllResponse>>(key);
      if(res)
      {
    ...
    return this.httpClient.get<MyPagedList<StudentGetAllResponse>>(`${this.apiUrl}`, {params}).pipe(tap((data) => {
      if(useCache) {
